...
def inference_gpt3(prompt, target_labels, engine="ada"):
    """
    ...
    # gpt3 always return token with whitespace as prefix
    prediction_token = response["choices"][0]["text"].strip(" ")
    assert prediction_token in target_labels, "top token not in target label set"
    prediction_dist = response["choices"][0]['logprobs']['top_logprobs'][0]

    return {"prediction_token": prediction_token, "prediction_dist": prediction_dist}
